...
class Detect(object):
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        smax = conf_preds[batch_idx, :, keep]
        ...
        if self.use_fast_nms:
            if self.use_cross_class_nms:
                ...
                boxes, masks, classes, scores, smax_pos = self.fast_nms(boxes, masks, scores, self.nms_thresh, self.top_k)
        else:
        ...
        smax = smax[:, smax_pos]
        return {'box': boxes, 'mask': masks, 'class': classes, 'score': scores, 'smax': smax}
    ...
    def fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200, second_threshold:bool=False):
        #print("Scores and sum: {}{}".format(scores.shape, torch.sum(scores, 0)))
        smax = scores
        positions = np.repeat(np.arange(scores.shape[1]), scores.shape[0])
        positions = positions.reshape(scores.shape[1],scores.shape[0])
        positions =  torch.as_tensor(positions.T, device=scores.device)
        #print(positions[0], positions.shape)
        scores, idx = scores.sort(1, descending=True)
        positions = torch.gather(positions, 1, idx)
        #print(positions)
        ...
        #print("Scores and prev {}{}".format(scores[:20], smax[:20][0]))
        #print("Scores new : {}".format(scores.shape))
        ...
        positions = positions[:cfg.max_num_detections]
        ...
        return boxes, masks, classes, scores, positions
