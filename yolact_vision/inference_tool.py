...
class InfTool:
    ...
    def raw_inference(self, img, preds=None, frame=None, batch_idx=0):
        ...
        # For yolact++ (for some reason returns 2 scores for each class (first one is higher))
        # First element in the list is box_scores, second mask_scores
        ...
        if preds[batch_idx]['detection']:
            smax = preds[batch_idx]['detection']['smax']
        else:
            smax = torch.Tensor()
        return classes, class_names, scores, boxes, masks, centroids, smax
